...
#
# LOGS:
#   Sin variable:   solo errores con explicación
#   BELL_DEBUG=1:   flujo completo por mensaje
#
# Uso:
#   python3 main.py              ← solo errores
#   BELL_DEBUG=1 python3 main.py ← flujo detallado
# ================================================
...
def _log(msg: str):
    """Solo imprime si BELL_DEBUG=1."""
    if _DEBUG:
        print(f'  {msg}')


def _error(capa: str, descripcion: str, exc: Exception = None):
    """Siempre imprime errores — con causa clara."""
...
def _procesar_mensaje(mensaje, socketio=None):
    ...
    try:
        ...
        conceptos = [
            ...
            for c in pc1.get('conceptos', [])
        ]
        descono = [
            (d.get('fragmento','') if isinstance(d,dict) else getattr(d,'fragmento',''))
            for d in pc1.get('desconocidos', [])
        ]
        _log(f'C1 conceptos: {conceptos}')
        if descono:
            _log(f'C1 desconocidos→zona: {descono}')
    ...
    try:
        ...
        primarios = [n.get('nodo_id','') for n in red.get('nodos_primarios', [])]
        _log(f'C2 nodos_primarios: {primarios}')
        _log(f'C2 total_activados: {red.get("total_activados", 0)}')

    except Exception as e:
        _error('CAPA 2', 'No pudo activar la red neuronal con los conceptos de C1', e)
    ...
    try:
        ...
        if pro.get('estado_subyacente'):
            _log(f'C3 estado_sub:    {pro["estado_subyacente"]} | energia: {pro.get("nivel_energia","?")}')
        if pro.get('dimensiones_dominantes'):
            _log(f'C3 dims_motor:    {pro["dimensiones_dominantes"]}')
        if pro.get('habilidad_requerida'):
            _log(f'C3 habilidad_req: {pro["habilidad_requerida"]}')

    except ImportError as e:
        _error('CAPA 3', 'Módulo de Capa 3 no encontrado — verificar imports de constructor_comprension o motor', e)
        resultado['respuesta'] = 'Capa 3 no disponible.'
        resultado['estado']    = 'parcial_capa2'
        return resultado
    except Exception as e:
        _error('CAPA 3', 'No pudo construir comprensión del mensaje', e)
    ...
    try:
        ...
        _error('CAPA 4', 'Fallo en evaluación de recursos y riesgo — usando fallback C3', e)
    ...
    try:
        ...
        sage  = delib.get('recomendacion_sage', '')[:60]
        _log(f'C5 tono_sage: {tono} | veto: {veto}')
        if sage:
            _log(f'C5 sage: "{sage}"')
        if veto:
            _log(f'C5 ⛔ VETO activado — respuesta directa')
        ...
        if socketio and pc5.get('deliberacion'):
            socketio.emit('capa5_deliberacion', {
                ...
                'veto':       veto,
                'tono_final': tono,
                'consejeras': len(delib.get('resultados', [])),
            })

    except Exception as e:
        _error('CAPA 5', 'Fallo en deliberación de consejeras — usando fallback C4', e)
    ...
    try:
        ...
        _error('CAPA 6', 'Fallo al construir respuesta de Bell — usando fallback C5', e)
    ...
    try:
        ...
        zona      = ejec.get('fue_a_zona', False)
        _log(f'C7 habilidad: {habilidad} | ejecuto: {ejecuto} | zona: {zona}')

        if socketio:
            socketio.emit('capa7_ejecucion', {
                'ejecuto': ejecuto, 'habilidad': habilidad, 'fue_a_zona': zona
            })

    except Exception as e:
        _error('CAPA 7', 'Fallo en ejecución de habilidad — usando respuesta C6', e)
    ...
    try:
        ...
        _log(f'C8 tono_final: {pc8.get("tono_final","?")} | tipo: {pc8.get("tipo_respuesta","?")}')
        ...
        _error('CAPA 8', 'Fallo en formato final — usando respuesta C7', e)
    ...
    try:
        ...
        if act.get('accion', 0) > 0 or act.get('relaciones', 0) > 0:
            ...
            _log(f'C9 BELL_CORE: accion={act.get("accion",0):.3f} relaciones={act.get("relaciones",0):.3f}')
            if socketio:
                socketio.emit('bell_core_actualizado', {
                    'accion':      act.get('accion', 0),
                    'relaciones':  act.get('relaciones', 0),
                    'crecimiento': act.get('crecimiento', 0),
                    'integridad':  act.get('integridad', 0),
                    'descripcion': act.get('descripcion', ''),
                })
        else:
            _log('C9 BELL_CORE: sin cambios')

    except ImportError as e:
        _error('CAPA 9', 'Módulo de Capa 9 no encontrado — usando respuesta C8', e)
        resultado['respuesta'] = pc8.get('respuesta_final', '')
        resultado['estado']    = 'parcial_capa8'
        return resultado
    except Exception as e:
        _error('CAPA 9', 'Fallo en integración — usando respuesta C8', e)
        resultado['respuesta'] = pc8.get('respuesta_final', '')
...
def _fb_c4(mensaje, pc2, pc3, pc4):
    ...
    return alternativa or random.choice([f"Aquí estoy, {nombre}.", f"Presente, {nombre}."])
